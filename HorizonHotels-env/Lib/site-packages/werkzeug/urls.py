...
class BaseURL(_URLTuple):
    ...
    @property
    def ***REMOVED***(self) -> t.Optional[str]:
        """The ***REMOVED*** if it was part of the URL, `None` otherwise.
        This undergoes URL decoding and will always be a string.
        """
    ...
    @property
    def raw_***REMOVED***(self) -> t.Optional[str]:
        """The ***REMOVED*** if it was part of the URL, `None` otherwise.
        Unlike :attr:`***REMOVED***` this one is not being decoded.
        """
        ...
        auth = ":".join(
            filter(
                ...
                [
                    ...
                    url_quote(self.raw_***REMOVED*** or "", "utf-8", "strict", "/:%"),
                ],
            )
        )
        if auth:
        ...
        auth = ":".join(
            filter(
                ...
                [
                    ...
                    _url_unquote_legacy(self.raw_***REMOVED*** or "", "/:%@"),
                ],
            )
        )
        if auth:
    ...
    def get_file_location(
        ...
        if host in ("***REMOVED***", "::1", "localhost"):
            host = None
    ...
    def _split_auth(self) -> t.Tuple[t.Optional[str], t.Optional[str]]:
        ...
        username, _, ***REMOVED*** = auth.partition(self._colon)
