...
class PasswordValueError(ValueError):
    """
    Error raised if a ***REMOVED*** can't be hashed / verified for various reasons.
    This exception derives from the builtin :exc:`!ValueError`.

    May be thrown directly when ***REMOVED*** violates internal invariants of hasher
...
class PasswordSizeError(PasswordValueError):
    """
    Error raised if a ***REMOVED*** exceeds the maximum size allowed
    by Passlib (by default, 4096 characters); or if ***REMOVED*** exceeds
    a hash-specific size limitation.

    This exception derives from :exc:`PasswordValueError` (above).

    Many ***REMOVED*** hash algorithms take proportionately larger amounts of time and/or
    memory depending on the size of the ***REMOVED*** provided. This could present
    a potential denial of service (DOS) situation if a maliciously large
    ***REMOVED*** is provided to an application. Because of this, Passlib enforces
    a maximum size limit, but one which should be *much* larger
    than any legitimate ***REMOVED***. :exc:`PasswordSizeError` derives
    from :exc:`!ValueError`.
    ...
    def __init__(self, max_size, msg=None):
        ...
        if msg is None:
            msg = "***REMOVED*** exceeds maximum allowed size"
...
class PasswordTruncateError(PasswordSizeError):
    """
    Error raised if ***REMOVED*** would be truncated by hash.
...
class UnknownHashError(ValueError):
    ...
    is unable to identify the algorithm used by a ***REMOVED*** hash.
...
def ExpectedTypeError(value, expected, param):
    ...
    # NOTE: value is never displayed, since it may sometimes be a ***REMOVED***.
...
def NullPasswordError(handler=None):
    """raised by OS crypt() supporting hashes, which forbid NULLs in ***REMOVED***"""
    name = _get_name(handler)
    return PasswordValueError("%s does not allow NULL bytes in ***REMOVED***" % name)
