...
class quirks:
    ...
    #: django check_***REMOVED***() started throwing error on encoded=None
    #: (really identify_hasher did)
    none_causes_check_***REMOVED***_error = DJANGO_VERSION >= (2, 1)

    #: django is_usable_***REMOVED***() started returning True for ***REMOVED*** = {None, ""} values.
    empty_is_usable_***REMOVED*** = DJANGO_VERSION >= (2, 1)

    #: django is_usable_***REMOVED***() started returning True for non-hash strings in 2.1
    invalid_is_usable_***REMOVED*** = DJANGO_VERSION >= (2, 1)
    ...
    def _create_django_hasher(self, django_name):
        ...
        if module is None or not module.adapter.patched:
            ...
            try:
                ...
                if not str(err).startswith("Unknown ***REMOVED*** hashing algorithm"):
                    raise
        else:
...
class DjangoContextAdapter(DjangoTranslator):
    ...
    #: ref to original make_***REMOVED***(),
    #: needed to generate usuable ***REMOVED***s that match django
    _orig_make_***REMOVED*** = None

    #: ref to django helper of this name -- not monkeypatched
    is_***REMOVED***_usable = None
    ...
    def __init__(self, context=None, get_user_category=None, **kwds):
        ...
        # get copy of original make_***REMOVED***
        from django.contrib.auth.hashers import make_***REMOVED***
        if make_***REMOVED***.__module__.startswith("passlib."):
            make_***REMOVED*** = _PatchManager.peek_unpatched_func(make_***REMOVED***)
        self._orig_make_***REMOVED*** = make_***REMOVED***

        # get other django helpers
        from django.contrib.auth.hashers import is_***REMOVED***_usable
        self.is_***REMOVED***_usable = is_***REMOVED***_usable
    ...
    #=============================================================================
    # django.contrib.auth.hashers helpers -- ***REMOVED*** helpers
    #=============================================================================

    def make_***REMOVED***(self, ***REMOVED***, salt=None, hasher="default"):
        """
        Passlib replacement for make_***REMOVED***()
        """
        if ***REMOVED*** is None:
            return self._orig_make_***REMOVED***(None)
        ...
        if "salt" not in passlib_hasher.setting_kwds:
            ...
            # Django make_***REMOVED***() autogenerates a salt if salt is bool False (None / ''),
            # so we only pass the keyword on if there's actually a fixed salt.
            passlib_hasher = passlib_hasher.using(salt=salt)
        return passlib_hasher.hash(***REMOVED***)

    def check_***REMOVED***(self, ***REMOVED***, encoded, setter=None, preferred="default"):
        """
        Passlib replacement for check_***REMOVED***()
        """
        # XXX: this currently ignores "preferred" keyword, since its purpose
        #      was for hash migration, and that's handled by the context.
        # XXX: honor "none_causes_check_***REMOVED***_error" quirk for django 2.2+?
        #      seems safer to return False.
        if ***REMOVED*** is None or not self.is_***REMOVED***_usable(encoded):
            return False

        # verify ***REMOVED***
        context = self.context
        try:
            correct = context.verify(***REMOVED***, encoded)
        ...
        if preferred == "default":
            if not context.needs_update(encoded, secret=***REMOVED***):
                return correct
        else:
            # Django's check_***REMOVED***() won't call setter() on a
            ...
            if (hasher.identify(encoded) and
                    not hasher.needs_update(encoded, secret=***REMOVED***)):
    ...
    def user_check_***REMOVED***(self, user, ***REMOVED***):
        """
        Passlib replacement for User.check_***REMOVED***()
        """
        if ***REMOVED*** is None:
            return False
        hash = user.***REMOVED***
        if not self.is_***REMOVED***_usable(hash):
            return False
        cat = self.get_user_category(user)
        try:
            ok, new_hash = self.context.verify_and_update(***REMOVED***, hash, category=cat)
        except exc.UnknownHashError:
            # As of django 1.5, unidentifiable hashes returns False
            # (side-effect of django issue 18453)
            return False
        if ok and new_hash is not None:
            ...
            user.***REMOVED*** = new_hash
            user.save()
        return ok

    def user_set_***REMOVED***(self, user, ***REMOVED***):
        """
        Passlib replacement for User.set_***REMOVED***()
        """
        if ***REMOVED*** is None:
            user.set_unusable_***REMOVED***()
        else:
            cat = self.get_user_category(user)
            user.***REMOVED*** = self.context.hash(***REMOVED***, category=cat)

    def get_user_category(self, user):
        """
        Helper for hashing ***REMOVED***s per-user --
    ...
    patch_locations = [
        ...
        #
        (USER_CLASS_PATH + ".check_***REMOVED***", "user_check_***REMOVED***", dict(method=True)),
        (USER_CLASS_PATH + ".set_***REMOVED***", "user_set_***REMOVED***", dict(method=True)),

        #
        # Hashers module
        #
        (HASHERS_PATH + ":", "check_***REMOVED***"),
        (HASHERS_PATH + ":", "make_***REMOVED***"),
        ...
        #
        (MODELS_PATH + ":", "check_***REMOVED***"),
        (MODELS_PATH + ":", "make_***REMOVED***"),
...
class _PasslibHasherWrapper(object):
    ...
    def verify(self, ***REMOVED***, encoded):
        return self.passlib_handler.verify(***REMOVED***, encoded)

    def encode(self, ***REMOVED***, salt=None, rounds=None, iterations=None):
        kwds = {}
...
##        # NOTE: from how make_***REMOVED***() is coded, all hashers
