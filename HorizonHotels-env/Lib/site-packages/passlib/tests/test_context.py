...
##        self.assertEqual(getdep(cc), ["md5_crypt"])
    ...
    def test_35_to_string(self):
        ...
        other = ctx.to_string(section="***REMOVED***-security")
        self.assertEqual(other, dump.replace("[passlib]","[***REMOVED***-security]"))
    ...
    def test_43_hash(self,):
        ...
        hash = cc.hash("***REMOVED***")
        self.assertTrue(hash.startswith("$5$rounds=3000$"))
        self.assertTrue(cc.verify("***REMOVED***", hash))
        self.assertFalse(cc.verify("***REMOVED***x", hash))
    ...
    def test_43_hash_legacy(self, use_16_legacy=False):
        ...
        with self.assertWarningList(["passing settings to.*is deprecated"]):
            self.assertEqual(
                cc.hash("***REMOVED***", scheme="phpass", salt='.'*8),
                '$H$5........De04R5Egz0aq8Tf.1eVhY/',
                )
        with self.assertWarningList(["passing settings to.*is deprecated"]):
            self.assertEqual(
                cc.hash("***REMOVED***", scheme="phpass", salt='.'*8, ident="P"),
        ...
        with self.assertWarningList(["passing settings to.*is deprecated"]):
            self.assertEqual(
                cc.hash("***REMOVED***", rounds=1999, salt="nacl"),
                '$5$rounds=1999$nacl$nmfwJIxqj0csloAAvSER0B8LU0ERCAbhmMug4Twl609',
                )

        with self.assertWarningList(["passing settings to.*is deprecated"]):
            self.assertEqual(
                cc.hash("***REMOVED***", rounds=2001, salt="nacl"),
    ...
    def test_47_verify_and_update(self):
        ...
        # create some hashes
        h1 = cc.handler("des_crypt").hash("***REMOVED***")
        h2 = cc.handler("sha256_crypt").hash("***REMOVED***")

        # check bad ***REMOVED***, deprecated hash
        ok, new_hash = cc.verify_and_update("wrongpass", h1)
        self.assertFalse(ok)
        self.assertIs(new_hash, None)

        # check bad ***REMOVED***, good hash
        ok, new_hash = cc.verify_and_update("wrongpass", h2)
        self.assertFalse(ok)
        self.assertIs(new_hash, None)

        # check right ***REMOVED***, deprecated hash
        ok, new_hash = cc.verify_and_update("***REMOVED***", h1)
        self.assertTrue(ok)
        self.assertTrue(cc.identify(new_hash), "sha256_crypt")

        # check right ***REMOVED***, good hash
        ok, new_hash = cc.verify_and_update("***REMOVED***", h2)
        self.assertTrue(ok)
        self.assertIs(new_hash, None)
