...
class _base_argon2_test(HandlerCase):
    ...
    known_correct_hashes = [
        ...
        # sample test
        ("***REMOVED***", '$argon2i$v=19$m=256,t=1,p=1$c29tZXNhbHQ$AJFIsNZTMKTAewB4+ETN1A'),

        # sample w/ all parameters different
        ("***REMOVED***", '$argon2i$v=19$m=380,t=2,p=2$c29tZXNhbHQ$SrssP8n7m/12VWPM8dvNrw'),
        ...
        ('***REMOVED***\x00', '$argon2i$v=19$m=512,t=2,p=2$c29tZXNhbHQ$Fb5+nPuLzZvtqKRwqUEtUQ'),

        # sample with type D (generated via argon_cffi2.PasswordHasher)
        ("***REMOVED***", '$argon2d$v=19$m=102400,t=2,p=8$g2RodLh8j8WbSdCp+lUy/A$zzAJqL/HSjm809PYQu6qkA'),
    ...
    def test_keyid_parameter(self):
        ...
        self.assertRaises(NotImplementedError, self.handler.verify, '***REMOVED***',
    ...
    def test_data_parameter(self):
        ...
        if self.backend == "argon2_cffi":
            ...
            self.assertRaises(NotImplementedError, handler.verify, "***REMOVED***", sample1)

            # incorrectly returns sample3, dropping data parameter
            self.assertEqual(handler.genhash("***REMOVED***", sample1), sample3)

        else:
            assert self.backend == "argon2pure"
            # should parse and verify
            self.assertTrue(handler.verify("***REMOVED***", sample1))

            # should preserve sample1
            self.assertEqual(handler.genhash("***REMOVED***", sample1), sample1)
        ...
        if self.backend == "argon2_cffi":
            ...
            # but v16.2 patches so it throws error on data parameter.
            # our code should detect that, and adapt it into a NotImplementedError
            self.assertRaises(NotImplementedError, handler.verify,"***REMOVED***", sample2)
            ...
            self.assertFalse(self.handler.verify("***REMOVED***", sample2))

            # should return sample1 (corrected digest)
            self.assertEqual(handler.genhash("***REMOVED***", sample2), sample1)
...
class argon2_argon2_cffi_test(_base_argon2_test.create_backend_case("argon2_cffi")):
    ...
    known_correct_hashes = _base_argon2_test.known_correct_hashes + [
        ...
        ('***REMOVED***', "$argon2i$m=65536,t=2,p=4$c29tZXNhbHQAAAAAAAAAAA$"
                     "QWLzI4TY9HkL2ZTLc8g6SinwdhZewYrzz9zxCo0bkGY"),

        # v1.3, type I
        ('***REMOVED***', "$argon2i$v=19$m=65536,t=2,p=4$c29tZXNhbHQ$"
                     "IMit9qkFULCMA/ViizL57cnTLOa5DiVM9eMwpAvPwr4"),

        # v1.3, type D
        ('***REMOVED***', "$argon2d$v=19$m=65536,t=2,p=4$c29tZXNhbHQ$"
                     "cZn5d+rFh+ZfuRhm2iGUGgcrW5YLeM6q7L3vBsdmFA0"),

        # v1.3, type ID
        ('***REMOVED***', "$argon2id$v=19$m=65536,t=2,p=4$c29tZXNhbHQ$"
        ...
        # ensure trailing null bytes handled correctly
        ('***REMOVED***\x00', "$argon2i$v=19$m=65536,t=2,p=4$c29tZXNhbHQ$"
