...
if has_min_django:
    ...
    class FakeUser(User):
        ...
        @memoized_property
        def saved_***REMOVED***s(self):
            return []

        def pop_saved_***REMOVED***s(self):
            try:
                return self.saved_***REMOVED***s[:]
            finally:
                del self.saved_***REMOVED***s[:]

        def save(self, update_fields=None):
            # NOTE: ignoring update_fields for test purposes
            self.saved_***REMOVED***s.append(self.***REMOVED***)
...
def check_django_hasher_has_backend(name):
    ...
    """
    assert name
    from django.contrib.auth.hashers import make_***REMOVED***
    try:
        make_***REMOVED***("", hasher=name)
        return True
...
def _modify_django_config(kwds, sha_rounds=None):
    ...
    # unchanged, instead of being upgraded in-place by check_***REMOVED***().
...
sample_hashes = dict(
    django_pbkdf2_sha256=("not a ***REMOVED***", django_pbkdf2_sha256
...
class _ExtensionSupport(object):
    ...
    def _iter_patch_candidates(cls):
        ...
        * whether it should or should not be patched
        ...
        user_attrs = ["check_***REMOVED***", "set_***REMOVED***"]
        model_attrs = ["check_***REMOVED***", "make_***REMOVED***"]
        hasher_attrs = ["check_***REMOVED***", "make_***REMOVED***", "get_hasher", "identify_hasher",
                        "get_hashers"]
    ...
    def assert_patched(self, context=None):
        ...
        if context is not None:
            ...
            self.assertEqual(mod.***REMOVED***_context.to_dict(resolve=True),
...
#: static ***REMOVED***s used by DjangoBehaviorTest methods
...
class DjangoBehaviorTest(_ExtensionTest):
    ...
    def assert_unusable_***REMOVED***(self, user):
        """
        check that user object is set to 'unusable ***REMOVED***' constant
        """
        self.assertTrue(user.***REMOVED***.startswith("!"))
        self.assertFalse(user.has_usable_***REMOVED***())
        self.assertEqual(user.pop_saved_***REMOVED***s(), [])

    def assert_valid_***REMOVED***(self, user, hash=UNSET, saved=None):
        """
        check that user object has a usable ***REMOVED*** hash.
        :param hash: optionally check it has this exact hash
        :param saved: check that mock commit history for user.***REMOVED*** matches this list
        """
        if hash is UNSET:
            self.assertNotEqual(user.***REMOVED***, "!")
            self.assertNotEqual(user.***REMOVED***, None)
        else:
            self.assertEqual(user.***REMOVED***, hash)
        self.assertTrue(user.has_usable_***REMOVED***(),
                        "hash should be usable: %r" % (user.***REMOVED***,))
        self.assertEqual(user.pop_saved_***REMOVED***s(),
    ...
    #   User.set_***REMOVED***()
    #   User.check_***REMOVED***()
    #   make_***REMOVED***() -- 1.4 only
    #   check_***REMOVED***()
    #   identify_hasher()
    #   User.has_usable_***REMOVED***()
    #   User.set_unusable_***REMOVED***()
    #
    ...
    def test_extension_config(self):
        ...
        from django.contrib.auth.hashers import check_***REMOVED***
        from passlib.ext.django.models import ***REMOVED***_context
        self.assertEqual(***REMOVED***_context.to_dict(resolve=True), ctx.to_dict(resolve=True))

        # should have patched both places
        from django.contrib.auth.models import check_***REMOVED*** as check_***REMOVED***2
        self.assertEqual(check_***REMOVED***2, check_***REMOVED***)
    ...
    def test_default_algorithm(self):
        ...
        # User.set_***REMOVED***() should use default alg
        user = FakeUser()
        user.set_***REMOVED***(PASS1)
        self.assertTrue(ctx.handler().verify(PASS1, user.***REMOVED***))
        self.assert_valid_***REMOVED***(user)

        # User.check_***REMOVED***() - n/a

        # make_***REMOVED***() should use default alg
        hash = make_***REMOVED***(PASS1)
        self.assertTrue(ctx.handler().verify(PASS1, hash))

        # check_***REMOVED***() - n/a

    def test_empty_***REMOVED***(self):
        """
        test how methods handle empty string as ***REMOVED***
        """
        ctx = self.context
        ...
        self.assert_valid_***REMOVED***(user, hash)

        # User.check_***REMOVED***() should return True
        self.assertTrue(user.check_***REMOVED***(""))
        self.assert_valid_***REMOVED***(user, hash)

        # XXX: test make_***REMOVED***() ?

        # TODO: is_***REMOVED***_usable()

        # identify_hasher() -- na

        # check_***REMOVED***() should return True
        self.assertTrue(check_***REMOVED***("", hash))
    ...
    def test_unusable_flag(self):
        ...
        # sanity check via user.set_unusable_***REMOVED***()
        user = FakeUser()
        user.set_unusable_***REMOVED***()
        self.assert_unusable_***REMOVED***(user)

        # ensure User.set_***REMOVED***() sets unusable flag
        user = FakeUser()
        user.set_***REMOVED***(None)
        self.assert_unusable_***REMOVED***(user)

        # User.check_***REMOVED***() should always fail
        self.assertFalse(user.check_***REMOVED***(None))
        self.assertFalse(user.check_***REMOVED***('None'))
        self.assertFalse(user.check_***REMOVED***(''))
        self.assertFalse(user.check_***REMOVED***(PASS1))
        self.assertFalse(user.check_***REMOVED***(WRONG1))
        self.assert_unusable_***REMOVED***(user)

        # make_***REMOVED***() should also set flag
        self.assertTrue(make_***REMOVED***(None).startswith("!"))

        # check_***REMOVED***() should return False (didn't handle disabled under 1.3)
        self.assertFalse(check_***REMOVED***(PASS1, '!'))

        # identify_hasher() and is_***REMOVED***_usable() should reject it
        self.assertFalse(is_***REMOVED***_usable(user.***REMOVED***))
        self.assertRaises(ValueError, identify_hasher, user.***REMOVED***)
    ...
    def test_none_hash_value(self):
        ...
        # User.set_***REMOVED***() - n/a

        # User.check_***REMOVED***() - returns False
        user = FakeUser()
        user.***REMOVED*** = None
        if quirks.none_causes_check_***REMOVED***_error and not patched:
            # django 2.1+
            self.assertRaises(TypeError, user.check_***REMOVED***, PASS1)
        else:
            self.assertFalse(user.check_***REMOVED***(PASS1))

        self.assertEqual(user.has_usable_***REMOVED***(),
                         quirks.empty_is_usable_***REMOVED***)

        # TODO: is_***REMOVED***_usable()

        # make_***REMOVED***() - n/a

        # check_***REMOVED***() - error
        if quirks.none_causes_check_***REMOVED***_error and not patched:
            self.assertRaises(TypeError, check_***REMOVED***, PASS1, None)
        else:
            self.assertFalse(check_***REMOVED***(PASS1, None))
    ...
    def test_empty_hash_value(self):
        ...
        # verify hash wasn't changed/upgraded during check_***REMOVED***() call
        self.assertEqual(user.***REMOVED***, "")
        self.assertEqual(user.pop_saved_***REMOVED***s(), [])

        # User.has_usable_***REMOVED***()
        self.assertEqual(user.has_usable_***REMOVED***(), quirks.empty_is_usable_***REMOVED***)
    ...
    def _do_test_invalid_hash_value(self, hash):
        ...
        # User.has_usable_***REMOVED***()
        self.assertEqual(user.has_usable_***REMOVED***(), quirks.invalid_is_usable_***REMOVED***)
        ...
        # check_***REMOVED***()
        self.assertFalse(check_***REMOVED***(PASS1, hash))
    ...
    def _do_test_available_scheme(self, scheme):
        ...
        try:
            ...
            while True:
                ...
                if secret:  # don't select blank ***REMOVED***s
                    break
        other = 'dontletmein'

        #-------------------------------------------------------
        # User.set_***REMOVED***() - not tested here
        #-------------------------------------------------------

        #-------------------------------------------------------
        # User.check_***REMOVED***()+migration against known hash
        #-------------------------------------------------------
        user = FakeUser()
        user.***REMOVED*** = hash

        # check against invalid ***REMOVED***
        self.assertFalse(user.check_***REMOVED***(None))
        ##self.assertFalse(user.check_***REMOVED***(''))
        self.assertFalse(user.check_***REMOVED***(other))
        self.assert_valid_***REMOVED***(user, hash)

        # check against valid ***REMOVED***
        self.assertTrue(user.check_***REMOVED***(secret))

        # check if it upgraded the hash
        # NOTE: needs_update kept separate in case we need to test rounds.
        needs_update = deprecated
        if needs_update:
            self.assertNotEqual(user.***REMOVED***, hash)
            self.assertFalse(handler.identify(user.***REMOVED***))
            self.assertTrue(ctx.handler().verify(secret, user.***REMOVED***))
            self.assert_valid_***REMOVED***(user, saved=user.***REMOVED***)
        else:
            self.assert_valid_***REMOVED***(user, hash)
        ...
        #-------------------------------------------------------
        self.assertTrue(is_***REMOVED***_usable(hash))
...
class DjangoExtensionTest(_ExtensionTest):
    ...
    def test_00_patch_control(self):
        """test set_django_***REMOVED***_context patch/unpatch"""
    ...
    def test_01_overwrite_detection(self):
        ...
        # mess with User.set_***REMOVED***, make sure it's detected
        orig = models.User.set_***REMOVED***
        models.User.set_***REMOVED*** = dummy
        with self.assertWarningList("another library has patched.*User\.set_***REMOVED***"):
            adapter._manager.check_all()
        models.User.set_***REMOVED*** = orig

        # mess with models.check_***REMOVED***, make sure it's detected
        orig = models.check_***REMOVED***
        models.check_***REMOVED*** = dummy
        with self.assertWarningList("another library has patched.*models:check_***REMOVED***"):
            adapter._manager.check_all()
        models.check_***REMOVED*** = orig
    ...
    def test_21_category_setting(self):
        ...
        def run(**kwds):
            """helper to take in user opts, return rounds used in ***REMOVED***"""
            user = FakeUser(**kwds)
            user.set_***REMOVED***("stub")
            return sha256_crypt.from_string(user.***REMOVED***).rounds
