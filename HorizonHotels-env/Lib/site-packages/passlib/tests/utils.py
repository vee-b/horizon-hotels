...
class HandlerCase(TestCase):
    """base class for testing ***REMOVED*** hash handlers (esp passlib.utils.handlers subclasses)
    ...
    # ***REMOVED***s used to test basic hash behavior - generally
    ...
    # forbid certain characters in ***REMOVED***s
    @classproperty
    ...
    def test_03_hash_workflow(self, use_16_legacy=False):
        ...
        the test is run against a couple of different stock ***REMOVED***s.
        """
        wrong_secret = 'stub'
        for secret in self.stock_***REMOVED***s:
            ...
            #
            # genhash() should NOT reproduce original hash for wrong ***REMOVED***
            #
    ...
    def test_secret_wo_truncate_size(self):
        """
        test no ***REMOVED*** size limits enforced (if truncate_size=None)
        """
        # skip if hasher has a maximum ***REMOVED*** size
        hasher = self.handler
        ...
        self.assertFalse(self.do_verify(alt_secret, hash),
                         "full ***REMOVED*** not used in digest")

    def test_secret_w_truncate_size(self):
        """
        test ***REMOVED*** size limits raise truncate_error (if appropriate)
        """
        #--------------------------------------------------
    ...
    def test_62_secret_border(self):
        """test non-string ***REMOVED***s are rejected"""
        hash = self.get_sample_hash()[1]
    ...
    # xxx: move to ***REMOVED*** size limits section, above?
    ...
    def test_64_forbidden_chars(self):
        """test forbidden characters not allowed in ***REMOVED***"""
    ...
    def test_77_fuzz_input(self, threaded=False):
        """fuzz testing -- random ***REMOVED***s and options
        ...
        * randomly generated ***REMOVED***s including extended unicode chars
        ...
        * runs output against other "external" verifiers such as OS crypt()
        ...
        count = 0
        while tick() <= stop:
            # generate random ***REMOVED*** & options
                ...
                if rng.random() < .1:
                    ...
                    if result and result != "skip":
                        raise self.failureException("was able to verify wrong "
                            "***REMOVED*** using %s: wrong_secret=%r real_secret=%r "
    ...
    def test_78_fuzz_threading(self):
        """multithreaded fuzz testing -- random ***REMOVED*** & options using multiple threads
    ...
    def get_fuzz_verifiers(self, threaded=False):
        """return list of ***REMOVED*** verifiers (including external libs)

        used by fuzz testing.
        verifiers should be callable with signature
        ``func(***REMOVED***: unicode, hash: ascii str) -> ok: bool``.
        """
        handler = self.handler
        verifiers = []
    ...
    class FuzzHashGenerator(object):
        ...
        ***REMOVED***s & configuration options to test hash with.
        ...
        # alphabet for randomly generated ***REMOVED***s
        ***REMOVED***_alphabet = u('qwertyASDF1234<>.@*#! \u00E1\u0259\u0411\u2113')

        # encoding when testing bytes
        ***REMOVED***_encoding = "utf-8"
        ...
        def generate(self):
            """
            generate random ***REMOVED*** and options for fuzz testing.
            :returns:
            ...
            def gendict(map):
                ...
                for key, meth in map.items():
                    ...
                    if value is not None:
                        out[key] = value
                return out
            secret, other = self.random_***REMOVED***_pair()
        ...
        #==========================================================
        # fuzz ***REMOVED*** generation
        #==========================================================
        def random_***REMOVED***_pair(self):
            """generate random ***REMOVED***, and non-matching alternate ***REMOVED***"""
            secret = self.random_***REMOVED***()
            while True:
                other = self.random_***REMOVED***()
                if self.accept_***REMOVED***_pair(secret, other):
                    break
            rng = self.rng
            if rng.randint(0,1):
                secret = secret.encode(self.***REMOVED***_encoding)
            if rng.randint(0,1):
                other = other.encode(self.***REMOVED***_encoding)
            return secret, other

        def random_***REMOVED***(self):
            """generate random ***REMOVED***s for fuzz testing"""
            # occasionally try an empty ***REMOVED***
            rng = self.rng
            if rng.random() < .0001:
                return u('')
            ...
            if max_size < 50 or rng.random() < .5:
                # chance of small ***REMOVED*** (~15 chars)
                size = self.randintgauss(1, min(max_size, 50), 15, 15)
            else:
                # otherwise large ***REMOVED*** (~70 chars)
                size = self.randintgauss(50, min(max_size, 99), 70, 20)

            # generate random ***REMOVED***
            result = getrandstr(rng, self.***REMOVED***_alphabet, size)
        ...
        def accept_***REMOVED***_pair(self, secret, other):
            """verify fuzz pair contains different ***REMOVED***s"""
            return secret != other
...
class OsCryptMixin(HandlerCase):
    ...
    * check that native crypt support is detected correctly for known platforms.
    ...
    def fuzz_verifier_crypt(self):
        ...
        encoding = self.FuzzHashGenerator.***REMOVED***_encoding
...
class UserHandlerMixin(HandlerCase):
    ...
    calls. as well, passing in a pair of strings as the ***REMOVED***
    ...
    def test_80_user(self):
        ...
        handler = self.handler
        ***REMOVED*** = 'stub'
        hash = handler.hash(***REMOVED***, user=self.default_user)

        if self.requires_user:
            self.assertRaises(TypeError, handler.hash, ***REMOVED***)
            self.assertRaises(TypeError, handler.genhash, ***REMOVED***, hash)
            self.assertRaises(TypeError, handler.verify, ***REMOVED***, hash)
        else:
            # e.g. cisco_pix works with or without one.
            handler.hash(***REMOVED***)
            handler.genhash(***REMOVED***, hash)
            handler.verify(***REMOVED***, hash)
...
class EncodingHandlerMixin(HandlerCase):
    ...
    calls by passing in a pair of strings as the ***REMOVED***
    ...
    # restrict stock ***REMOVED***s & fuzz alphabet to latin-1,
    ...
    class FuzzHashGenerator(HandlerCase.FuzzHashGenerator):
        ...
        ***REMOVED***_alphabet = u('qwerty1234<>.@*#! \u00AC')
