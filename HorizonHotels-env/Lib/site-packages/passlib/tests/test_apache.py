...
def _call_htpasswd_verify(path, user, ***REMOVED***):
    """
    wrapper for htpasswd verify
    """
    out, rc = _call_htpasswd(["-vi", path, user], ***REMOVED***)
    return not rc
...
class HtpasswdFileTest(TestCase):
    ...
    def test_00_constructor_autoload(self):
        ...
        # NOTE: "default_scheme" option checked via set_***REMOVED***() test, among others
    ...
    def test_02_set_***REMOVED***(self):
        """test set_***REMOVED***()"""
        ht = apache.HtpasswdFile.from_string(
            self.sample_01, default_scheme="plaintext")
        self.assertTrue(ht.set_***REMOVED***("user2", "pass2x"))
        self.assertFalse(ht.set_***REMOVED***("user5", "pass5"))
        ...
        # invalid user
        self.assertRaises(ValueError, ht.set_***REMOVED***, "user:", "pass")

        # test that legacy update() still works
        with self.assertWarningList("update\(\) is deprecated"):
            ht.update("user2", "test")
        self.assertTrue(ht.check_***REMOVED***("user2", "test"))

    def test_02_set_***REMOVED***_autosave(self):
        path = self.mktemp()
        sample = b'user1:pass1\n'
        set_file(path, sample)

        ht = apache.HtpasswdFile(path)
        ht.set_***REMOVED***("user1", "pass2")
        self.assertEqual(get_file(path), sample)
    ...
    def test_02_set_***REMOVED***_default_scheme(self):
        """test set_***REMOVED***() -- default_scheme"""

        def check(scheme):
            ht = apache.HtpasswdFile(default_scheme=scheme)
            ht.set_***REMOVED***("user1", "pass1")
    ...
    def test_03_users(self):
        """test users()"""
        ht = apache.HtpasswdFile.from_string(self.sample_01)
        ht.set_***REMOVED***("user5", "pass5")
        ht.delete("user3")
        ht.set_***REMOVED***("user3", "pass3")
        self.assertEqual(sorted(ht.users()), ["user1", "user2", "user3", "user4", "user5"])

    def test_04_check_***REMOVED***(self):
        """test check_***REMOVED***()"""
        ht = apache.HtpasswdFile.from_string(self.sample_05)
        self.assertRaises(TypeError, ht.check_***REMOVED***, 1, 'pass9')
        self.assertTrue(ht.check_***REMOVED***("user9","pass9") is None)
        ...
        for i in irange(1, 7):
            i = str(i)
            try:
                self.assertTrue(ht.check_***REMOVED***("user"+i, "pass"+i))
                self.assertTrue(ht.check_***REMOVED***("user"+i, "pass9") is False)
        ...
        self.assertRaises(ValueError, ht.check_***REMOVED***, "user:", "pass")
    ...
    def test_05_load(self):
        ...
        hc.load(path)
        self.assertTrue(hc.check_***REMOVED***('user1','pass1'))
    ...
    def test_htpasswd_cmd_verify(self):
        ...
        # base scheme
        ht.set_hash("user1", hash_scheme("***REMOVED***","apr_md5_crypt"))

        # 2.2-compat scheme
        host_no_bcrypt = apache.htpasswd_defaults["host_apache_22"]
        ht.set_hash("user2", hash_scheme("***REMOVED***", host_no_bcrypt))

        # 2.4-compat scheme
        host_best = apache.htpasswd_defaults["host"]
        ht.set_hash("user3", hash_scheme("***REMOVED***", host_best))
        ...
        self.assertTrue(_call_htpasswd_verify(path, "user1", "***REMOVED***"))
        self.assertTrue(_call_htpasswd_verify(path, "user2", "***REMOVED***"))
        self.assertTrue(_call_htpasswd_verify(path, "user3", "***REMOVED***"))
    ...
    def test_htpasswd_cmd_verify_bcrypt(self):
        ...
        def hash_scheme(pwd, scheme):
            return ht.context.handler(scheme).hash(pwd)
        ht.set_hash("user1", hash_scheme("***REMOVED***", "bcrypt"))
        ht.save()
        self.assertFalse(_call_htpasswd_verify(path, "user1", "wrong"))
        if HAVE_HTPASSWD_BCRYPT:
            ...
            self.assertFalse(_call_htpasswd_verify(path, "user1", "***REMOVED***"))
...
class HtdigestFileTest(TestCase):
    ...
    def test_02_set_***REMOVED***(self):
        """test update()"""
        ht = apache.HtdigestFile.from_string(self.sample_01)
        self.assertTrue(ht.set_***REMOVED***("user2", "realm", "pass2x"))
        self.assertFalse(ht.set_***REMOVED***("user5", "realm", "pass5"))
        self.assertEqual(ht.to_string(), self.sample_03)

        # default realm
        self.assertRaises(TypeError, ht.set_***REMOVED***, "user2", "pass3")
        ht.default_realm = "realm2"
        ht.set_***REMOVED***("user2", "pass3")
        ht.check_***REMOVED***("user2", "realm2", "pass3")

        # invalid user
        self.assertRaises(ValueError, ht.set_***REMOVED***, "user:", "realm", "pass")
        self.assertRaises(ValueError, ht.set_***REMOVED***, "u"*256, "realm", "pass")

        # invalid realm
        self.assertRaises(ValueError, ht.set_***REMOVED***, "user", "realm:", "pass")
        self.assertRaises(ValueError, ht.set_***REMOVED***, "user", "r"*256, "pass")
    ...
    # TODO: test set_***REMOVED*** autosave

    def test_03_users(self):
        """test users()"""
        ht = apache.HtdigestFile.from_string(self.sample_01)
        ht.set_***REMOVED***("user5", "realm", "pass5")
        ht.delete("user3", "realm")
        ht.set_***REMOVED***("user3", "realm", "pass3")
    ...
    def test_04_check_***REMOVED***(self):
        """test check_***REMOVED***()"""
        ht = apache.HtdigestFile.from_string(self.sample_01)
        self.assertRaises(TypeError, ht.check_***REMOVED***, 1, 'realm', 'pass5')
        self.assertRaises(TypeError, ht.check_***REMOVED***, 'user', 1, 'pass5')
        self.assertIs(ht.check_***REMOVED***("user5", "realm","pass5"), None)
        for i in irange(1,5):
            i = str(i)
            self.assertTrue(ht.check_***REMOVED***("user"+i, "realm", "pass"+i))
            self.assertIs(ht.check_***REMOVED***("user"+i, "realm", "pass5"), False)

        # default realm
        self.assertRaises(TypeError, ht.check_***REMOVED***, "user5", "pass5")
        ht.default_realm = "realm"
        self.assertTrue(ht.check_***REMOVED***("user1", "pass1"))
        self.assertIs(ht.check_***REMOVED***("user5", "pass5"), None)
        ...
        # invalid user
        self.assertRaises(ValueError, ht.check_***REMOVED***, "user:", "realm", "pass")
    ...
    def test_05_load(self):
        ...
        # make changes, check load_if_changed() does nothing
        ha.set_***REMOVED***("user1", "realm", "pass1")
        ha.load_if_changed()
        ...
        ha.set_***REMOVED***("user5", "realm", "pass5")
        ha.load()
        self.assertEqual(ha.to_string(), self.sample_01)
    ...
    def test_06_save(self):
        ...
        # test save w/ no path
        hb = apache.HtdigestFile()
        hb.set_***REMOVED***("user1", "realm", "pass1")
        self.assertRaises(RuntimeError, hb.save)
