...
class _DjangoHelper(TestCase):
    ...
    def test_90_django_reference(self):
        ...
        for secret, hash in self.iter_known_hashes():
            self.assertTrue(check_***REMOVED***(secret, hash),
                            "secret=%r hash=%r failed to verify" %
                            (secret, hash))
            self.assertFalse(check_***REMOVED***('x' + secret, hash),
                            "mangled secret=%r hash=%r incorrect verified" %
                            (secret, hash))

    def test_91_django_generation(self):
        """test against output of Django's make_***REMOVED***()"""
        self._require_django_support()
        # XXX: esp. when it's no longer supported by django,
        #      should verify it's *NOT* recognized
        ...
        while tick() < end:
            secret, other = generator.random_***REMOVED***_pair()
            if not secret: # django rejects empty ***REMOVED***s.
                continue
            if isinstance(secret, bytes):
                secret = secret.decode("utf-8")
            hash = make_***REMOVED***(secret, hasher=name)
...
class django_disabled_test(HandlerCase):
    ...
    known_alternate_hashes = [
        ...
        ("!9wa845vn7098ythaehasldkfj", "***REMOVED***", "!"),
    ]
...
class django_des_crypt_test(HandlerCase, _DjangoHelper):
    ...
    known_correct_hashes = [
        ...
        ("***REMOVED***",         'crypt$c2$c2M87q...WWcU'),
        ("***REMOVED***",         'crypt$c2e86$c2M87q...WWcU'),
        ("***REMOVED***ignoreme", 'crypt$c2.AZ$c2M87q...WWcU'),
    ...
    known_alternate_hashes = [
        ...
        ('crypt$$c2M87q...WWcU', "***REMOVED***", 'crypt$c2$c2M87q...WWcU'),
    ]
...
class django_salted_md5_test(HandlerCase, _DjangoHelper):
    ...
    known_correct_hashes = [
        ...
        ("***REMOVED***",    'md5$123abcdef$c8272612932975ee80e8a35995708e80'),
...
class django_salted_sha1_test(HandlerCase, _DjangoHelper):
    ...
    known_correct_hashes = [
        # test extra large salt
        ("***REMOVED***",'sha1$123abcdef$e4a1877b0e35c47329e7ed7e58014276168a37ba'),
...
class django_argon2_test(HandlerCase, _DjangoHelper):
    ...
    known_correct_hashes = [
        # sample test
        ("***REMOVED***", 'argon2$argon2i$v=19$m=256,t=1,p=1$c29tZXNhbHQ$AJFIsNZTMKTAewB4+ETN1A'),

        # sample w/ all parameters different
        ("***REMOVED***", 'argon2$argon2i$v=19$m=380,t=2,p=2$c29tZXNhbHQ$SrssP8n7m/12VWPM8dvNrw'),
