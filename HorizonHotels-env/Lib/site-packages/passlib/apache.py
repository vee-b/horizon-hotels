"""passlib.apache - apache ***REMOVED*** support"""
...
class _CommonFile(object):
    ...
    # FIXME: htpasswd doc says ***REMOVED***s limited to 255 chars under Windows & MPE,
...
class HtpasswdFile(_CommonFile):
    ...
    :param encoding:
        ...
        and hash ***REMOVED***s. Defaults to ``utf-8``, though ``latin-1``
    ...
    :param default_scheme:
        Optionally specify default scheme to use when encoding new ***REMOVED***s.
    ...
    :param default:
        Change the default algorithm used to hash new ***REMOVED***s.
    ...
    ================
    .. automethod:: users
    .. automethod:: check_***REMOVED***
    .. automethod:: get_hash

    Modification
    ================
    .. automethod:: set_***REMOVED***
    .. automethod:: delete
    ...
    def set_***REMOVED***(self, user, ***REMOVED***):
        """Set ***REMOVED*** for user; adds user if needed.
        ...
        """
        hash = self.context.hash(***REMOVED***)
        return self.set_hash(user, hash)

    @deprecated_method(deprecated="1.6", removed="1.8",
                       replacement="set_***REMOVED***")
    def update(self, user, ***REMOVED***):
        """set ***REMOVED*** for user"""
        return self.set_***REMOVED***(user, ***REMOVED***)
    ...
    def check_***REMOVED***(self, user, ***REMOVED***):
        """
        Verify ***REMOVED*** for specified user.
        If algorithm marked as deprecated by CryptContext, will automatically be re-hashed.

        :returns:
            * ``None`` if user not found.
            * ``False`` if user found, but ***REMOVED*** does not match.
            * ``True`` if user found and ***REMOVED*** matches.
        ...
        if hash is None:
            return None
        if isinstance(***REMOVED***, unicode):
            # NOTE: encoding ***REMOVED*** to match file, making the assumption
            # that server will use same encoding to hash the ***REMOVED***.
            ***REMOVED*** = ***REMOVED***.encode(self.encoding)
        ok, new_hash = self.context.verify_and_update(***REMOVED***, hash)
        if ok and new_hash is not None:
            # rehash user's ***REMOVED*** if old hash was deprecated
    ...
    @deprecated_method(deprecated="1.6", removed="1.8",
                       replacement="check_***REMOVED***")
    def verify(self, user, ***REMOVED***):
        """verify ***REMOVED*** for user"""
        return self.check_***REMOVED***(user, ***REMOVED***)
...
class HtdigestFile(_CommonFile):
    ...
    .. automethod:: realms
    .. automethod:: users
    .. automethod:: check_***REMOVED***(user[, realm], ***REMOVED***)
    .. automethod:: get_hash

    Modification
    ============
    .. automethod:: set_***REMOVED***(user[, realm], ***REMOVED***)
    .. automethod:: delete
    ...
    def set_***REMOVED***(self, user, realm=None, ***REMOVED***=_UNSET):
        """Set ***REMOVED*** for user; adds user & realm if needed.

        If ``self.default_realm`` has been set, this may be called
        with the syntax ``set_***REMOVED***(user, ***REMOVED***)``,
        otherwise it must be called with all three arguments:
        ``set_***REMOVED***(user, realm, ***REMOVED***)``.
        ...
        if ***REMOVED*** is _UNSET:
            # called w/ two args - (user, ***REMOVED***), use default realm
            realm, ***REMOVED*** = None, realm
        realm = self._require_realm(realm)
        hash = htdigest.hash(***REMOVED***, user, realm, encoding=self.encoding)
        return self.set_hash(user, realm, hash)

    @deprecated_method(deprecated="1.6", removed="1.8",
                       replacement="set_***REMOVED***")
    def update(self, user, realm, ***REMOVED***):
        """set ***REMOVED*** for user"""
        return self.set_***REMOVED***(user, realm, ***REMOVED***)
    ...
    def check_***REMOVED***(self, user, realm=None, ***REMOVED***=_UNSET):
        """Verify ***REMOVED*** for specified user + realm.

        If ``self.default_realm`` has been set, this may be called
        with the syntax ``check_***REMOVED***(user, ***REMOVED***)``,
        otherwise it must be called with all three arguments:
        ``check_***REMOVED***(user, realm, ***REMOVED***)``.
        ...
        if hash is None:
            return None
        return htdigest.verify(***REMOVED***, hash, user, realm,
                               encoding=self.encoding)

    @deprecated_method(deprecated="1.6", removed="1.8",
                       replacement="check_***REMOVED***")
    def verify(self, user, realm, ***REMOVED***):
        """verify ***REMOVED*** for user"""
        return self.check_***REMOVED***(user, realm, ***REMOVED***)
