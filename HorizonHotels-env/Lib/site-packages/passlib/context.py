...
class CryptContext(object):
    """Helper for hashing & verifying ***REMOVED***s using multiple algorithms.
    ...
    def needs_update(self, hash, scheme=None, category=None, secret=None):
        ...
        If so, the ***REMOVED*** should be re-hashed using :meth:`hash`
    ...
    def hash(self, secret, scheme=None, category=None, **kwds):
        ...
        :param category:
            Optional :ref:`user category <user-categories>`.
            If specified, this will cause any category-specific defaults to
            be used when hashing the ***REMOVED*** (e.g. different default scheme,
    ...
    def verify(self, secret, hash, scheme=None, category=None, **kwds):
        ...
        It will then check whether the ***REMOVED*** verifies against the hash.
        ...
        :returns:
            ``True`` if the ***REMOVED*** matched the hash, else ``False``.
    ...
    def verify_and_update(self, secret, hash, scheme=None, category=None, **kwds):
        """verify ***REMOVED*** and re-hash the ***REMOVED*** if needed, all in a single call.

        This is a convenience method which takes care of all the following:
        first it verifies the ***REMOVED*** (:meth:`~CryptContext.verify`), if this is successfull
        it checks if the hash needs updating (:meth:`~CryptContext.needs_update`), and if so,
        re-hashes the ***REMOVED*** (:meth:`~CryptContext.hash`), returning the replacement hash.
        ...
        :param category:
            Optional :ref:`user category <user-categories>`.
            If specified, this will cause any category-specific defaults to
            be used if the ***REMOVED*** has to be re-hashed.
        ...
        :returns:
            ...
            flag indicating whether the ***REMOVED*** verified,
    ...
    def dummy_verify(self, elapsed=0):
        ...
        in order to simulate time it would take to hash a ***REMOVED***.

        Runs verify() against a dummy hash, to simulate verification
        of a real account ***REMOVED***.
    ...
    def is_enabled(self, hash):
        """
        test if hash represents a usuable ***REMOVED*** --
        i.e. does not represent an unusuable ***REMOVED*** such as ``"!"``,
    ...
    def disable(self, hash=None):
        ...
        :returns:
            ...
            but is guaranteed to not validate against *any* ***REMOVED***.
        """
    ...
    def enable(self, hash):
        ...
        thus restoring the user's original ***REMOVED***.
