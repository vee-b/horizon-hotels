...
class _BcryptCommon(uh.SubclassBackendMixin, uh.TruncateMixin, uh.HasManyIdents,
    ...
    def _finalize_backend_mixin(mixin_cls, backend, dryrun):
        ...
        def safe_verify(secret, hash):
            ...
            try:
                ...
                #       - PasswordValueError if there's encoding issue w/ ***REMOVED***.
        ...
        def detect_wrap_bug(ident):
            ...
            NOTE: reference hash is of ***REMOVED*** "0"*72
    ...
    def _norm_digest_args(cls, secret, ident, new=False):
        ...
        # silently truncate the ***REMOVED*** at first NULL they encounter!
        if _BNULL in secret:
        ...
        if ident == IDENT_2A:
            ...
            if cls._lacks_20_support:
                ...
                # by repeating the ***REMOVED*** until it's at least 72 chars in length.
                if secret:
                    if require_valid_utf8_bytes:
                        # NOTE: this only works because bcrypt will ignore everything past
                        #       secret[71], so padding to include a full utf8 sequence
        ...
        if PY3 and isinstance(secret, bytes):
            try:
                ...
                raise error_from(uh.exc.PasswordValueError(
                    "python3 crypt.crypt() ony supports bytes ***REMOVED***s using UTF8; "
...
class bcrypt(_NoBackend, _BcryptCommon):
    """This class implements the BCrypt ***REMOVED*** hash, and follows the :ref:`***REMOVED***-hash-api`.
    ...
    :param bool truncate_error:
        By default, BCrypt will silently truncate ***REMOVED***s larger than 72 bytes.
...
class _wrapped_bcrypt(bcrypt):
    ...
    def _check_truncate_policy(cls, secret):
        # disable check performed by bcrypt(), since this doesn't truncate ***REMOVED***s.
        pass
...
class bcrypt_sha256(_wrapped_bcrypt):
    ...
    def _calc_checksum(self, secret):
        ...
        # NOTE: bcrypt doesn't fully mix entropy for bytes 55-72 of ***REMOVED***
        ...
        if self.version == 1:
            ...
            # this prevents known secret -> sha256 ***REMOVED*** tables from being
