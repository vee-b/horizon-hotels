...
class unix_fallback(uh.ifc.DisabledHash, uh.StaticHandler):
    """This class provides the fallback behavior for unix shadow files, and follows the :ref:`***REMOVED***-hash-api`.
    ...
    * for security, ***REMOVED***s will always hash to ``!``.
    * it rejects all ***REMOVED***s if the hash is NOT an empty string (``!`` or ``*`` are frequently used).
    * by default it rejects all ***REMOVED***s if the hash is an empty string,
      but if ``enable_wildcard=True`` is passed to verify(),
      all ***REMOVED***s will be allowed through if the hash is an empty string.
...
class unix_disabled(uh.ifc.DisabledHash, uh.MinimalHandler):
    """This class provides disabled ***REMOVED*** behavior for unix shadow files,
    and follows the :ref:`***REMOVED***-hash-api`.

    This class does not implement a hash, but instead matches the "disabled account"
    strings found in ``/etc/shadow`` on most Unix variants. "encrypting" a ***REMOVED***
    will simply return the disabled account marker. It will reject all ***REMOVED***s,
    ...
    @classmethod
    def identify(cls, hash):
        # NOTE: technically, anything in the /etc/shadow ***REMOVED*** field
        ...
        #       * linux counts empty string as "any ***REMOVED***";
    ...
    def genhash(cls, secret, config, marker=None):
        if not cls.identify(config):
            ...
            # preserve the existing str,since it might contain a disabled ***REMOVED*** hash ("!" + hash)
            uh.validate_secret(secret)
...
class plaintext(uh.MinimalHandler):
    """This class stores ***REMOVED***s in plaintext, and follows the :ref:`***REMOVED***-hash-api`.
    ...
    :param encoding:
        ...
        This encoding will be used to encode :class:`!unicode` ***REMOVED***s
