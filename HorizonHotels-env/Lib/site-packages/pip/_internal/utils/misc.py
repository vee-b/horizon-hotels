...
def ask_***REMOVED***(message):
    # type: (str) -> str
    """Ask for a ***REMOVED*** interactively."""
    _check_no_input(message)
...
def split_auth_from_netloc(netloc):
    ...
    Returns: (netloc, (username, ***REMOVED***)).
    """
    if "@" not in netloc:
    ...
    # the ***REMOVED*** attribute of urlsplit()'s return value).
    auth, netloc = netloc.rsplit("@", 1)
    pw = None  # type: Optional[str]
    if ":" in auth:
        # Split from the left because that's how urllib.parse.urlsplit()
        # behaves if more than one : is present (which again can be checked
        # using the ***REMOVED*** attribute of the return value)
...
def redact_netloc(netloc):
    ...
    """
    netloc, (user, ***REMOVED***) = split_auth_from_netloc(netloc)
    if user is None:
        return netloc
    if ***REMOVED*** is None:
        user = "****"
        ...
        ***REMOVED*** = ":****"
    return "{user}{***REMOVED***}@{netloc}".format(
        user=user, ***REMOVED***=***REMOVED***, netloc=netloc
    )
...
def split_auth_netloc_from_url(url):
    ...
    Returns: (url_without_auth, netloc, (username, ***REMOVED***))
    """
...
def remove_auth_from_url(url):
    # type: (str) -> str
    """Return a copy of url with 'username:***REMOVED***@' removed."""
...
def redact_auth_from_url(url):
    # type: (str) -> str
    """Replace the ***REMOVED*** in a given url with ****."""
