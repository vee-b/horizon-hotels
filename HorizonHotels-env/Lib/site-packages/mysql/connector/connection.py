...
class MySQLConnection(MySQLConnectionAbstract):
    ...
    def _do_auth(self, username=None, ***REMOVED***=None, database=None,
        ...
        Raises NotSupportedError when we get the old, insecure ***REMOVED***
    ...
    def _auth_switch_request(self, username=None, ***REMOVED***=None):
        ...
        if packet[4] == 254 and len(packet) == 5:
            raise errors.NotSupportedError(
                "Authentication with old (insecure) ***REMOVED***s "
            ...
            auth = get_auth_plugin(new_auth_plugin)(
                auth_data, ***REMOVED***=***REMOVED***, ssl_enabled=self._ssl_active)
        ...
        if packet[4] == 1:
            ...
            auth = get_auth_plugin(new_auth_plugin)(
                auth_data, ***REMOVED***=***REMOVED***, ssl_enabled=self._ssl_active)
            if new_auth_plugin == "caching_sha2_***REMOVED***":
                response = auth.auth_response()
                if response:
                    self._socket.send(response)
                packet = self._socket.recv()
    ...
    def _open_connection(self):
        ...
        try:
            ...
            self._do_handshake()
            self._do_auth(self._user, self._***REMOVED***,
    ...
    def cmd_change_user(self, username='', ***REMOVED***='', database='',
                        charset=45):
        ...
        ok_packet = self._auth_switch_request(username, ***REMOVED***)
    ...
    def reset_session(self, user_variables=None, session_variables=None):
        ...
        try:
            ...
            self.cmd_change_user(self._user, self._***REMOVED***,
